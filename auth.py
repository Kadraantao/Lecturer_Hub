"""Authentication helpers: password hashing, sign up, sign in."""
import hashlib
import secrets
...
def create_session(user_id: int, days: int = 30) -> str:
    """Generate a session token, store it in the DB, return the token."""
...
def delete_session(token: str):
    """Remove a session from the DB (called on sign out)."""
    if not token:
        return
    conn = get_connection()
...
def hash_password(password: str, salt: Optional[str] = None):
    """Hash a password using PBKDF2-SHA256. Returns (hash_hex, salt_hex)."""
    if salt is None:
...
def sign_up(email: str, password: str, full_name: str):
    """Create a student account. Returns (ok, message)."""
...
def sign_in(email: str, password: str):
    """Return a user dict on success, None on failure."""
    email = (email or "").strip().lower()
